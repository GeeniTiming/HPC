...
int main(int argc, char** argv) {
    ...
    if (argc < 2) {
        if (rank == 0) printf("usage: %s N [threads] [seed]\n", argv[0]);
        MPI_Finalize();
        return 0;
    }

    int N = atoi(argv[1]);
    int threads_arg = (argc >= 3) ? atoi(argv[2]) : 0;
    ...
    if (threads_arg > 0) omp_set_num_threads(threads_arg);
    ...
    if (rank == 0) {
        ...
        double efficiency = speedup / ((double)size * (double)threads);
        printf("N=%d procs=%d threads=%d time=%.6f speedup=%.6f efficiency=%.6f max_error=%.3e\n", N, size, threads, par_time, speedup, efficiency, max_err);
    }
